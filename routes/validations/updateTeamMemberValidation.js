...
const updateTeamMemberProfileValidationWithStatus = (reqBody) => {
    const schema = Joi.object({
        firstName: Joi.string().min(3).max(20).empty().required().messages({
            "string.base": "\"firstName\" should be a type of 'text'",
            "string.min": "\"firstName\" should have a minimum length of 3 ", // {#limit}
...
module.exports = { updateTeamMemberProfileValidation, updateTeamMemberProfileValidationWithStatus, updatephoneNumberValidation, updatePasswordValidation, updateStatusValidation, updateTeachingLevelValidation };
/* status: Joi.array().min(1) */