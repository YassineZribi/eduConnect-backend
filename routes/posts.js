...
// @desc    *** Get all posts ***
// @access  *** Private ***
router.get("/:childhoodInstitutionId/:skip/:limit", authPrivRoutes, async (req, res) => {
    try {
        ...
        if (userToAccess.childhoodInstitution.toString() === req.params.childhoodInstitutionId) {                                                              // childhoodInstitution: on peut ajouter ce field avec "user <ici>" mais je trouve que c'est inutile et représente un gaspillage en performance.
            const posts = await Post.find({ childhoodInstitution: req.user.childhoodInstitution }).sort({ date: -1 }).skip(Number(req.params.skip)).limit(Number(req.params.limit)).populate([{ path: "comments", populate: { path: "user" } }, { path: "childhoodInstitution", select: "institutionName logo" }, { path: "comments", populate: { path: "responses.userIsTeamMember", model: "TeamMember" } }, { path: "comments", populate: { path: "responses.userIsParent", model: "Parent" } }]).populate([{ path: "likes.userIsParent", model: "Parent" }, { path: "likes.userIsTeamMember", model: "TeamMember" }, { path: "dislikes.userIsParent", model: "Parent" }, { path: "dislikes.userIsTeamMember", model: "TeamMember" }, { path: "comments", populate: { path: "likes.userIsTeamMember", model: "TeamMember" } }, { path: "comments", populate: { path: "likes.userIsParent", model: "Parent" } }, { path: "comments", populate: { path: "dislikes.userIsTeamMember", model: "TeamMember" } }, { path: "comments", populate: { path: "dislikes.userIsParent", model: "Parent" } }, { path: "comments", populate: { path: "responses.likes.userIsTeamMember", model: "TeamMember" } }, { path: "comments", populate: { path: "responses.likes.userIsParent", model: "Parent" } }, { path: "comments", populate: { path: "responses.dislikes.userIsTeamMember", model: "TeamMember" } }, { path: "comments", populate: { path: "responses.dislikes.userIsParent", model: "Parent" } }]);
...
router.put("/add_like_to_response/:comment_id/:response_id", authPrivRoutes, async (req, res) => {
    try {
        ...
        let comment = await Comment.findOne({ _id: req.params.comment_id, childhoodInstitution: req.user.childhoodInstitution });
        if (!comment) return res.status(404).json({ errorMsg: "post not found" });
        ...
        comment.responses[findIndex].likes.unshift({ user: req.user.id, userIsParent: req.user.id, userIsTeamMember: req.user.id, onModel: userIs });
        if (userIs === "TeamMember") {
            comment = await Comment.populate(comment, [{ path: "responses.likes.userIsTeamMember" }, { path: "responses.dislikes.userIsTeamMember" }]);
        } else if (userIs === "Parent") {
            comment = await Comment.populate(comment, [{ path: "responses.likes.userIsParent" }, { path: "responses.dislikes.userIsParent" }]);
        }
        await comment.save();
...
router.put("/add_dislike_to_response/:comment_id/:response_id", authPrivRoutes, async (req, res) => {
    try {
        ...
        comment.responses[findIndex].dislikes.unshift({ user: req.user.id, userIsParent: req.user.id, userIsTeamMember: req.user.id, onModel: userIs });
        if (userIs === "TeamMember") {
            comment = await Comment.populate(comment, [{ path: "responses.dislikes.userIsTeamMember" }, { path: "responses.likes.userIsTeamMember" }]);
        } else if (userIs === "Parent") {
            comment = await Comment.populate(comment, [{ path: "responses.dislikes.userIsParent" }, { path: "responses.likes.userIsParent" }]);
        }
        await comment.save();
