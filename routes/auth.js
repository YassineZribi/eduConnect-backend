...
const loginUserValidation = require('./validations/loginParentValidation');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(400).json({ alert: 'Email or Password is wrong' }); // userNotFound: 'Email is not found'

        // checking if the password entered and the user password saved in the databse are equal
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) return res.status(401).json({ alert: 'Email or Password is wrong' }); // invalidPassword: 'Invalid Password'
